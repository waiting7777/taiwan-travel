...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    // add cat
    this.cat = this.add.image(800, 200, 'cat')
    this.cat.setScale(0.7)
    console.log(this.cat)

    // add welcome text
    this.welcome1 = this.add.text(350, 100, 'Hi~歡迎來到', { fontSize: '24px', color: '#000' })
    this.welcome2 = this.add.text(340, 130, '台灣好好玩', { fontSize: '32px', fontWeight: 'bold', color: '#000' })
    this.welcome3 = this.add.text(335, 170, '景點尋寶積分賽', { fontSize: '24px', color: '#000' })

    // add arrow down button
    this.arrowButton = this.add.image(600, 475, 'arrow-down').setInteractive({ useHandCursor: true  })
    // event listener for the background
    this.arrowButton.on('pointerdown', this.nextPage, this);

    // add taiwan
    this.taiwan = this.add.image(600, 280, 'taiwan').setScale(0.4).setAlpha(0)

    // add oh bear
    this.ohBear = this.add.image(-125, 300, 'oh-bear').setScale(0.7).setAngle(-45)

    // add score
    this.score = this.add.image(1150, 400, 'score').setScale(0.4, 0)
